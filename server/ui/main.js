...
var STORAGE_KEY = "stackflow-console-config-v1";
var connectedAddress = null;
var stackflowNodeCounterpartyEnabled = false;
var stackflowNodeCounterpartyPrincipal = null;
var peerRequestCounter = 0;
var ids = {
...
function updateActionUi() {
  ...
  if (mySigLabel) {
    mySigLabel.textContent = signAction ? "My Signature (Generated Output)" : "My Signature (RSV hex)";
  }
  if (mySigHelp) {
    mySigHelp.textContent = signAction ? "Click the submit button to generate this signature. It will auto-fill here." : "Paste your signature, or switch to a sign-* action to generate it here.";
  }
  if (isCounterpartyRequestAction(action) && !normalizedText(getInput(ids.sigWith).value) && stackflowNodeCounterpartyPrincipal) {
  ...
  if (isCounterpartyRequestAction(action)) {
    if (stackflowNodeCounterpartyEnabled && stackflowNodeCounterpartyPrincipal) {
      ...
      counterpartyHint = " Counterparty signing is not reported as enabled by the server.";
    }
  }
...
function parseActionContext({ requireNonce = false } = {}) {
  ...
  const token = normalizedText(getInput(ids.sigToken).value) || null;
  const nonce = requireNonce ? toBigInt(getInput(ids.sigNonce).value, "Nonce") : null;
  return {
    withPrincipal,
    token,
    nonce
...
async function buildStructuredState() {
  ...
  const balance1 = pair.principal1 === connectedAddress ? signer.myBalance : signer.theirBalance;
  const balance2 = pair.principal1 === connectedAddress ? signer.theirBalance : signer.myBalance;
...
function extractAddress(response) {
  const isStacksAddress = (value) => typeof value === "string" && /^S[PMT][A-Z0-9]{38,42}$/i.test(value);
  ...
  const findAddress = (value) => {
    ...
    if (Array.isArray(value)) {
      for (const item of value) {
        if (item && typeof item === "object" && String(item.symbol || item.chain || "").toUpperCase().includes("STX") && isStacksAddress(item.address)) {
          return item.address;
        }
      }
      for (const item of value) {
...
function buildStackflowNodePayload() {
  ...
  const amount = parsed.action === 2n || parsed.action === 3n ? toBigInt(getInput(ids.callFundAmount).value, "Amount").toString(10) : "0";
  return {
    contractId,
...
function buildCounterpartyRequestPayload(action) {
  ...
  const amount = config.action === "2" || config.action === "3" ? toBigInt(getInput(ids.callFundAmount).value, "Amount").toString(10) : "0";
  return {
...
function renderPayloadPreview() {
  const action = getSelectedAction();
  if (action === "sign-transfer" || action === "sign-deposit" || action === "sign-withdrawal" || action === "sign-close") {
    ...
    if (mySignature) {
      ...
      $(ids.signaturePayload).textContent = "Generated signature appears here.";
    }
    return;
  }
  if (isCounterpartyRequestAction(action)) {
    try {
      const request2 = buildCounterpartyRequestPayload(action);
      $(ids.signaturePayload).textContent = JSON.stringify(request2, null, 2);
    } catch (error) {
      $(ids.signaturePayload).textContent = error instanceof Error ? error.message : "invalid counterparty request";
    }
    return;
  }
  if (action !== "submit-signature-state") {
    $(ids.signaturePayload).textContent = "Payload preview appears for submit-signature-state and counterparty requests.";
    return;
  }
  try {
    ...
    $(ids.signaturePayload).textContent = error instanceof Error ? error.message : "invalid signature payload";
  }
}
...
function renderPipesPlaceholder(message) {
  $(ids.pipesBody).innerHTML = `<article class="pipe-card pipe-card-empty"><p>${escapeHtml(message)}</p></article>`;
}
...
function computeDisplayBalances(pipe, connected) {
  ...
  const mineConfirmed = connectedIs1 ? pipe.balance1 : connectedIs2 ? pipe.balance2 : null;
  const theirsConfirmed = connectedIs1 ? pipe.balance2 : connectedIs2 ? pipe.balance1 : null;
  const minePending = connectedIs1 ? pipe.pending1Amount : connectedIs2 ? pipe.pending2Amount : null;
  const theirsPending = connectedIs1 ? pipe.pending2Amount : connectedIs2 ? pipe.pending1Amount : null;
  const minePendingHeight = connectedIs1 ? pipe.pending1BurnHeight : connectedIs2 ? pipe.pending2BurnHeight : null;
  const theirsPendingHeight = connectedIs1 ? pipe.pending2BurnHeight : connectedIs2 ? pipe.pending1BurnHeight : null;
  ...
  const mineEffective = mineConfirmedUint !== null && minePendingUint !== null ? (mineConfirmedUint + minePendingUint).toString(10) : mineConfirmed;
  const theirsEffective = theirsConfirmedUint !== null && theirsPendingUint !== null ? (theirsConfirmedUint + theirsPendingUint).toString(10) : theirsConfirmed;
  return {
    counterparty,
    mineConfirmed,
    theirsConfirmed,
...
async function fetchJson(url, init2) {
...
function pipeMatchesParticipants(pipe, connected, withPrincipal, token) {
  ...
  return pipeToken === token && (principal1 === connected && principal2 === withPrincipal || principal2 === connected && principal1 === withPrincipal);
}
async function resolvePipeTotals(withPrincipal, token) {
  ...
  const pipe = pipes.find(
    (candidate) => pipeMatchesParticipants(candidate, connectedAddress, withPrincipal, token)
  );
  if (!pipe) {
...
async function requestCounterpartySignature(action) {
  ...
  const body = await response.json().catch(() => ({}));
  if (response.status === 409 && body?.reason === "nonce-too-low") {
    const incomingNonce = body?.incomingNonce ?? requestPayload.payload.nonce ?? "?";
    const existingNonce = body?.existingNonce ?? body?.state?.nonce ?? "?";
    setStatus(
      ids.txResult,
...
function normalizeNetworkName(value) {
  ...
  if (text === "mainnet" || text === "testnet" || text === "devnet" || text === "mocknet") {
    return text;
  }
  return null;
}
async function syncNetworkFromStackflowNode() {
  ...
  try {
    ...
    stackflowNodeCounterpartyPrincipal = typeof health?.counterpartyPrincipal === "string" && normalizedText(health.counterpartyPrincipal) ? health.counterpartyPrincipal : null;
